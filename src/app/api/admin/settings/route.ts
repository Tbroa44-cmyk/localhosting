...
export async function POST(request: Request) {
  try {
    ...
    if (!current) {
      await db.prepare(
        "INSERT INTO settings (id, trading_enabled, trading_open_hour, trading_close_hour, emergency_close, emergency_message, trading_days, bots_enabled) VALUES (1, ?, ?, ?, ?, ?, ?, ?)"
      ).run(
      ...
      if (body.bots_enabled !== undefined) { updates.push("bots_enabled = ?"); values.push(body.bots_enabled); }
      if (updates.length > 0) {
        values.push(1);
