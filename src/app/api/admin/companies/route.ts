...
export async function GET() {
  try {
    ...
    let users: any[] = [];
    try {
      users = await db.prepare("SELECT * FROM users ORDER BY created_at DESC").all() as any[];
      users = (Array.isArray(users) ? users : []).map((u: any) => ({
        id: u.id, username: u.username, email: u.email, balance: u.balance ?? 0,
        is_admin: u.is_admin ?? 0, created_at: u.created_at,
        allowed: u.allowed ?? 1,
        role: u.role ?? "user",
