  ...
  try {
    ...
    if (session?.user) {
      ...
      for (const o of myPendingOrders as any[]) {
        ...
        myTrades.push({ type: String(o.type), shares: normalizedShares, original_shares: Number(o.original_shares) || Math.max(0, normalizedShares), price_per_share: o.price_per_share, total_amount: normalizedShares * o.price_per_share, created_at: o.created_at, status: normalizedShares <= 0 ? "confirmed" : "pending", order_id: o.id, is_market_order: isMarketOrderRow(o) ? 1 : 0 });
      }
    ...
    return NextResponse.json({
      ...
      pending_buy_orders: graphBuyRows.map((o) => ({ shares: orderBookShares(o), created_at: o.created_at, is_market_order: isMarketOrderRow(o) ? 1 : 0 })),
      pending_sell_orders: graphSellRows.map((o) => ({ shares: orderBookShares(o), created_at: o.created_at, is_market_order: isMarketOrderRow(o) ? 1 : 0 })),
    }, {
